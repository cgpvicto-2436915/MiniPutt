...
public class CibleCamera : MonoBehaviour
{
    ...
    private void DeplacerCamera()
    {
        if (deplacement.sqrMagnitude > 0.0f)
        {
            Vector3 prochainePosition = transform.position +
                transform.right * deplacement.y * Time.deltaTime -
                transform.forward * deplacement.x * Time.deltaTime;
    ...
    {
        float angle = (transform.localEulerAngles.z + inclinaison * Time.deltaTime) % 360;

        if (angle < limitesInclinaison.x || angle > limitesInclinaison.y)
        {
            transform.Rotate(new Vector3(0.0f, 0.0f, inclinaison * Time.deltaTime), Space.Self);
        }
    }
    #endregion
